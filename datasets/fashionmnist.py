...
class ValidAugmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        image = self.data[index]  # 假设形状是 (28, 28)
        if image.ndim == 2:  # 如果没有通道维度
            image = image.unsqueeze(0)  # 添加通道维度，形状变为 (1, 28, 28)
        image = (self.strong_transform
                 (image[index]))
...
# 加载 fashion 数据集
def load_fashionmnist(batch_size, partial_rate):

    processed_folder = 'processed'
    training_file = 'training.pt'
    test_file = 'test.pt'

    train_data, train_labels = torch.load(os.path.join('./data/fashion/', processed_folder, training_file))
    train_data = train_data.numpy()

    test_data, test_labels = torch.load(os.path.join('./data/fashion/', processed_folder, test_file))
    ...
    # 创建静态测试集
    test_dataset = TestAugmentation(test_data, test_labels)
    test_loader = data.DataLoader(test_dataset, batch_size=batch_size * 4, shuffle=False, num_workers=0)
